...
def MG_triplet(model, graph, feat, optimizer, max_epoch, device, adata_concat_, pis_list_, scheduler, logger=None, key_="MG_triplet", iter_comb=None):
    ...
    # print(section_ids)
    # print(adata_concat_[0])
    # print(adata_concat_[1])

    # with torch.no_grad():
    #     z = model.embed(graph, x)
    # print(pis_list_.shape)
...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        args_mask_rate = args.mask_rate
        args_remask_rate = args.remask_rate
        if is_consecutive == 1:
            # use_cached_pi = False
            ...
            for i in range(args.num_class):
                #sub_slices1.append(slice1[slice1.obs['mclust']==i])
                #sub_slices1.append(slice2[slice2.obs['mclust']==i])
                subslice1 = slice1[slice1.obs['mclust']==i]
                subslice2 = slice2[slice2.obs['mclust']==i]
                if subslice1.shape[0]>0 and subslice2.shape[0]>0:
                    ...
                    # temp_local_pi_list.append(local_PI)
                    # print(G)
                    # print(G.shape)
        ...
        # if use_cached_pi:
        #     global_PI = None
            # print(global_PI.shape)
            # layerwise_pct = cal_layer_based_alignment_result(global_PI, slice1, slice2)
            # print(layerwise_pct)
            # accuracy_localOT_array = np.array([layerwise_pct])
            # np.savetxt(os.path.join(exp_fig_dir, "_".join(section_ids)+".txt"), accuracy_localOT_array, delimiter=',')
            # print(global_G)
        ...
        if is_consecutive:
            ...
            logging.info("Keep training Model with MSE + triplet loss ")
            ...
            # TODO, parallel implementation if without hard priors
    ...
    # if logger is not None:
    #     logger.finish()
    # return (np.mean(ari_1) + np.mean(ari_2)) / 2  # for grid search usage as final metric 
    return 0
